...
#kerala bus
lists_KB=[]
df_KB=pd.read_csv("kerala_bus_details.csv")
for i,r in df_KB.iterrows():
    lists_KB.append(r["Route_Name"])
...
if web == "📍States and Routes":
    S = slt.selectbox("Lists of States", [ "Andhra Pradesh", "Telegana", "kadamba", "Rajastan", 
                                          "Chandigarh", "Himachal", "Assam", "Jammu", "West Bengal","Kerala"])
...
# Kerala bus fare filtering
    if S=="Kerala":
        KB=slt.selectbox("list of routes",lists_KB)

        def type_and_Price_KB(Bus_Type, Price_range):
            conn = mysql.connector.connect(host="localhost", user="root", password="naga", database="RED_BUS_DETAILS")
            my_cursor = conn.cursor()
            # Define fare range based on selection
            if Price_range == "50-1000":
        ...
        df_result = type_and_Price_KB(select_type, select_Price)
        slt.dataframe(df_result)
